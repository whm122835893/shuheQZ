...
export class SynthesisService {
  ...
  async synthesize(userId: number, activityId: number, dto: SynthesizeDto) {
    return this.dataSource.transaction(async (manager) => {
      ...
      //    使用 SELECT ... FOR UPDATE 锁定活动记录，串行化同一活动的并发合成请求，
      //    防止 per_user_limit / total_limit 检查的 TOCTOU 竞态
      ...
      await manager.save(NftOperationLog, {
        ...
        newValue: {
          ...
          user_id: userId,
          result_collectible_id: resultCollectible.id,
          result_user_collectible_id: Number(savedUserCollectible.id),
          consumed_user_collectible_ids: ids,
          serial_no: serialNo,
        },
        ip: null,
        isDelete: 0,
      ...
      // 12) // TODO: 触发异步 mint
      // this.logger.log(`TODO: trigger async mint for user_collectible ${savedUserCollectible.id}`);
