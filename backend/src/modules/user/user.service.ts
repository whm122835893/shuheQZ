...
export class UserService implements IUserService {
  ...
  private aesEncrypt(plain: string): string {
    ...
    if (!rawKey) {
      ...
      console.warn('[AES] DATA_AES_KEY 未配置，使用开发默认密钥（仅限开发环境）');
    }
  ...
  private async callThirdPartyRealname(
    ...
    if (devMode) {
      console.warn(
        '[Realname] REALNAME_DEV_MODE=true，跳过实名认证（仅限开发联调，生产环境必须设置为 false）',
      );
      return true;
    }
    ...
    if (!kycProvider) {
      console.error(
        '[Realname] KYC_PROVIDER 未配置，实名认证不可用。请配置 KYC 服务商或设置 REALNAME_DEV_MODE=true 进行联调。',
      );
      return false;
    }
    ...
    console.error(`[Realname] KYC_PROVIDER=${kycProvider} 尚未实现，实名认证失败`);
    return false;
  }

  /** 查询进行中的抽奖活动（status=1 且在时间范围内） */
