...
export class WalletService {
  ...
  async handleRechargeCallback(dto: RechargeCallbackDto): Promise<void> {
    try {
      ...
      // INT-011 修复：Redis SETNX 原子幂等锁，防止并发回调双重入账
      const idempotencyKey = `wallet:recharge:idempotency:${dto.transaction_no}`;
      const lockResult = await this.redis.setnx(idempotencyKey, Date.now().toString());
      if (lockResult !== 1) {
        this.logger.log(
      ...
      // 设置 1 小时 TTL，防止 key 永久残留
      await this.redis.expire(idempotencyKey, 3600);
