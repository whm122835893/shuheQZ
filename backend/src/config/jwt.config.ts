...
export const getJwtConfig = (configService: ConfigService): JwtConfig => {
  ...
  if (!secret) {
    console.warn('[JWT] JWT_SECRET 未配置，使用开发默认值（仅限开发环境）');
  }
  if (!refreshSecret) {
    console.warn('[JWT] JWT_REFRESH_SECRET 未配置，使用开发默认值（仅限开发环境）');
  }
...
/**
 * 同步获取 JWT 配置对象（基于 process.env，用于无法注入 ConfigService 的场景）
 */
const isProd = process.env.NODE_ENV === 'production';
if (isProd && !process.env.JWT_SECRET) {
  throw new Error('[JWT] 生产环境必须配置 JWT_SECRET 环境变量');
}
if (isProd && !process.env.JWT_REFRESH_SECRET) {
  throw new Error('[JWT] 生产环境必须配置 JWT_REFRESH_SECRET 环境变量');
}
