...
import { getQualificationCollectibles } from '../../api/salePlan'

// 藏品列表（来自后端），用于补全资格购配置的藏品信息
const collectibleList = ref<any[]>([])

// 仅显示发售计划中设置为资格购模式的藏品
const qualificationPlans = computed(() => getQualificationCollectibles())
...
async function loadData() {
  ...
  // 构建资格购配置列表（从发售计划获取，无后端接口时为空列表）
  const plans = getQualificationCollectibles()
  qualList.value = plans.map((p) => {
    ...
    return {
      ...
      valid_range: `${p.onsale_at} 至 ${p.end_at}`,
      status: p.status === 'on_sale' ? '生效中' : p.status === 'scheduled' ? '未开始' : p.status === 'ended' ? '已结束' : '未开始',
      phones: []
    }
  })
}
...
function openEdit(row: QualItem) {
  ...
  Object.assign(configForm, {
    ...
    price: row.price,
    valid_range: row.valid_range.split(' 至 ')
  })
  configVisible.value = true
}
