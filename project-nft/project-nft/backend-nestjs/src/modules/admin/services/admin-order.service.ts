...
export class AdminOrderService {
  ...
  async findList(query: {
    ...
    // 关联查询用户和藏品信息，避免 N+1 问题
    qb.leftJoinAndMapOne('o.user', NftUser, 'u', 'u.id = o.user_id')
      .leftJoinAndMapOne('o.collectible', NftCollectible, 'c', 'c.id = o.collectible_id');
  ...
  async markPaid(id: number, adminId: number, dto: { paymentMethod?: string; remark?: string }): Promise<any> {
    ...
    try {
      ...
      payment.transactionNo = `MANUAL${now.getTime()}${Math.floor(Math.random() * 10000)}`;
  ...
  async initiateRefund(id: number, adminId: number, dto: { reason: string; amount?: number }): Promise<NftRefund> {
    ...
    refund.refundNo = `RF${Date.now()}${Math.floor(Math.random() * 10000)}`;
