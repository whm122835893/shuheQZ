...
async function submitForm() {
  ...
  await formRef.value.validate(async (valid) => {
    if (!valid) return
    if (editingRow.value) {
      const result = await updateCategoryName(editingRow.value.id, form.name)
      if (result.success) {
      ...
      const result = await addCategory(form.name)
      if (result.success) {
        const newCat = categories.value[categories.value.length - 1]
        if (newCat && !form.enabled) {
          await toggleCategoryEnabled(newCat.id, false)
        }
        ElMessage.success(result.message)
        dialogVisible.value = false
      } else {
...
async function handleMoveUp(id: number) {
  await moveCategory(id, 'up')
}

async function handleMoveDown(id: number) {
  await moveCategory(id, 'down')
}
