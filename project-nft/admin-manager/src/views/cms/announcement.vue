...
async function fetchData() {
  ...
  try {
    ...
    if (searchForm.title) params.keyword = searchForm.title
    if (searchForm.type) params.type = searchForm.type
    if (searchForm.status) params.status = searchForm.status
    const res = await cmsApi.announcements(params)
    pageData.value = {
      list: (res?.list || []).map((item: any) => ({
        ...
        image: item.image || '',
        status: item.is_delete === 0 ? (item.status || 'draft') : 'offline',
        publish_time: item.publishTime || item.publish_time || item.created_at || '',
...
async function handleTopChange(row: AnnItem, val: boolean) {
  try {
    await cmsApi.updateAnnouncement(row.id, { is_top: val })
...
async function handleToggleOnline(row: AnnItem) {
  ...
  try {
    ...
    await cmsApi.togglePublishAnnouncement(row.id)
  ...
  await formRef.value.validate(async (valid) => {
    ...
    try {
      ...
      if (mode === 'draft') {
        ...
        payload.publish_time = form.scheduled_time
      } else {
        payload.status = 'published'
      }

      if (isEdit.value) {
        await cmsApi.updateAnnouncement(form.id, payload)
        ElMessage.success('公告已更新')
      } else {
        await cmsApi.createAnnouncement(payload)
        ElMessage.success(mode === 'scheduled' ? `公告已设置定时发布，将于 ${form.scheduled_time} 自动发布` : '公告已发布')
      }
...
async function handleDelete(row: AnnItem) {
  try {
    ...
    await cmsApi.deleteAnnouncement(row.id)
