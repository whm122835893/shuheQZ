...
async function handleResetPwd(row: UserInfo) {
  ...
  try {
    await userApi.resetTxPassword(row.id)
    ElMessage.success(`已重置用户「${row.nickname}」的交易密码`)
  } catch (e: any) {
...
async function handleForceLogout(row: UserInfo) {
  ...
  try {
    await userApi.forceLogout(row.id)
...
function handleExport() {
  ...
  const header = ['ID', '昵称', '手机号', '注册时间', '账号状态', '钱包余额', '藏品数量']
  ...
  ElMessage.success(`已导出 ${list.length} 条用户数据`)
}
