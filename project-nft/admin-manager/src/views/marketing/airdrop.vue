...
async function fetchData() {
  ...
  try {
    ...
    const res = await marketingApi.airdrop(params)
    ...
    pageData.value = {
      list: list.map((item: any) => ({
        ...
        airdropTime: item.airdropTime || item.airdrop_time || item.createdAt || ''
      })),
      total: res?.total || 0
    }
  } catch (e) {
...
async function handleAirdrop() {
  ...
  // 查找所选藏品/盲盒的ID
  let collectibleId = 0
  if (airdropForm.rewardType === 'collectible') {
    const found = availableCollectibles.value.find(c => c.name === airdropForm.rewardContent)
    collectibleId = found?.id || 0
  } else if (airdropForm.rewardType === 'blindbox') {
    const found = availableBlindboxes.value.find(c => c.name === airdropForm.rewardContent)
    collectibleId = found?.id || 0
  }
  if ((airdropForm.rewardType === 'collectible' || airdropForm.rewardType === 'blindbox') && !collectibleId) {
    ElMessage.warning('无法找到所选藏品/盲盒的ID，请重新选择')
    return
  }
  ...
  try {
    // 1. 创建空投活动
    const createRes = await marketingApi.createAirdrop({
      name: `空投-${airdropForm.rewardContent}-${Date.now()}`,
      type: 'direct',
      collectibleId,
      quantityPerUser: airdropForm.quantity,
      totalLimit: phones.length * airdropForm.quantity,
      description: `向 ${phones.length} 个用户空投 ${airdropForm.rewardContent}`
    }) as any
    const airdropId = createRes?.id || createRes?.data?.id
    if (!airdropId) {
      throw new Error('创建空投活动失败：未返回活动ID')
    }
    // 2. 执行空投
    await marketingApi.executeAirdrop(airdropId)
