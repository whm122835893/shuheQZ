...
async function loadData() {
  ...
  try {
    ...
    localList.value = result.list.map((b: BlacklistRaw) => ({
      id: Number(b.id),
      user_id: Number(b.userId ?? b.user_id ?? b.operatorId ?? 0),
      nickname: b.nickname || '',
      phone: b.phone || b.targetValue || '',
      reason: b.reason || '',
      operator: b.operator || (b.operatorId ? `管理员#${b.operatorId}` : ''),
...
async function confirmAdd() {
  ...
  await formRef.value.validate(async (valid) => {
    ...
    try {
      await securityApi.blacklistAdd({
        type: 4,
        target: addForm.phone,
  ...
  ElMessageBox.confirm(
    ...
    .then(async () => {
      try {
        ...
        ElMessage.error(e?.message || '移出黑名单失败')
      }
    })
    .catch(() => {})
}
