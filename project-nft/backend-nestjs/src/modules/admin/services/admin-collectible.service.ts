...
export class AdminCollectibleService {
  ...
  // ------------------------------------------------------------
  // 说明：
  //  - 复用已注入的 categoryRepo: Repository<NftCategory>
  //  - is_delete 兼作"启用/禁用"状态：0=启用，1=禁用
  //  - 实体 code 字段为必填且唯一（idx_category_code），
  //    创建时若未传入 code 则自动生成
  // ============================================================

  /** 分类表名，用于操作日志 */
  private static readonly CATEGORY_TABLE = 'nft_categories';

  /** 生成分类 code（保证唯一，长度 <=20 以匹配 varchar(20)） */
  private generateCategoryCode(): string {
    const ts = Date.now().toString(36);
    const rand = Math.random().toString(36).slice(2, 6);
    return `C${ts}${rand}`.slice(0, 20);
  }

  /** 查询分类，不存在则抛 404（不按 is_delete 过滤，以便切换启用/禁用） */
  private async findCategoryOrThrow(id: number): Promise<NftCategory> {
  ...
  async findCategories(query: any) {
    ...
    // 默认返回全部（含禁用），便于管理后台切换启用/禁用；可通过 isDelete 过滤
    if (query.isDelete !== undefined && query.isDelete !== '') {
      qb.andWhere('cat.is_delete = :isDelete', {
        isDelete: Number(query.isDelete),
      });
    }

    qb.orderBy('cat.sort_order', 'ASC')
      .addOrderBy('cat.created_at', 'DESC')
      .offset((page - 1) * pageSize)
      .limit(pageSize);
  ...
  async createCategory(data: {
    ...
    if (!data?.name) {
      throw new BadRequestException('缺少必要参数：name');
    }
    // code 在实体中为必填且唯一，若未传入则自动生成
    let code = data.code || this.generateCategoryCode();
    // 防止极小概率的 code 冲突
    if (await this.categoryRepo.findOne({ where: { code } })) {
    ...
    try {
      ...
      this.logger.error(`创建分类失败: ${err?.message ?? err}`);
      throw new HttpException(
        '创建分类失败，请稍后重试',
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
  ...
  async updateCategory(
    id: number,
    data: { name?: string; sortOrder?: number; icon?: string },
  ) {
    await this.findCategoryOrThrow(id);
    const update: Partial<NftCategory> = {};
    if (data.name !== undefined) {
      update.name = data.name;
    }
    if (data.sortOrder !== undefined) {
      update.sortOrder = Number(data.sortOrder);
    }
  ...
  // C5. 切换分类启用/禁用状态（切换 is_delete）
  async toggleCategory(id: number) {
    const category = await this.findCategoryOrThrow(id);
    const newValue = category.isDelete === 1 ? 0 : 1;
    const now = new Date();
    const setFields: Record<string, unknown> = { isDelete: newValue };
    if (newValue === 1) {
  ...
  async reorderCategories(items: Array<{ id: number; sortOrder: number }>) {
    ...
    try {
      ...
      await queryRunner.rollbackTransaction();
      this.logger.error(`批量重排序分类失败: ${err.message}`, err.stack);
      throw new HttpException(
        '批量重排序失败，请稍后重试',
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
