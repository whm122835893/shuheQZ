...
//
// 说明：
//   - 用户黑名单通过 nft_blacklist 表维护（blacklist_type=1 用户），NftUser 无 is_blacklisted 字段
...
export class AdminUserService {
  ...
  async findList(query: any) {
    ...
    // 黑名单过滤（基于 nft_blacklist 子查询）
    const isBl = String(query.isBlacklisted ?? '');
    if (isBl === '1') {
      qb.andWhere(
        `u.id IN (SELECT CAST(bl.target_value AS UNSIGNED) FROM nft_blacklist bl WHERE bl.blacklist_type = 1 AND bl.status = 1)`,
      );
    } else if (isBl === '0') {
      qb.andWhere(
        `u.id NOT IN (SELECT CAST(bl.target_value AS UNSIGNED) FROM nft_blacklist bl WHERE bl.blacklist_type = 1 AND bl.status = 1)`,
      );
    }
  ...
  async findOne(id: number) {
    ...
    const blacklistEntry = await this.blacklistRepo.findOne({
      where: { blacklistType: 1, targetValue: String(id), status: 1 },
    });
    return {
      ...user,
