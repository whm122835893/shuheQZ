...
export class AdminSecurityService {
  ...
  async getBlacklist(query: Record<string, any>): Promise<PaginatedResult<NftBlacklist>> {
    ...
    if (query.type !== undefined && query.type !== '') {
      qb.andWhere('b.blacklistType = :type', { type: Number(query.type) });
    }
    if (query.target) {
      qb.andWhere('b.targetValue LIKE :t', { t: `%${query.target}%` });
    }
  ...
  async createBlacklist(
    ...
    const exist = await this.blacklistRepo.findOne({
      where: { blacklistType: Number(body.type), targetValue: body.target, status: 1 },
      select: ['id'],
    });
    if (exist) {
    ...
    const row = this.blacklistRepo.create({
      blacklistType: Number(body.type),
      targetValue: body.target,
      reason: body.reason ?? '',
      operatorId: admin.id,
      expireAt: body.expiredAt ? new Date(body.expiredAt) : null,
      status: 1,
    });
  ...
  async getRiskAlertList(query: Record<string, any>): Promise<PaginatedResult<NftRiskAlert>> {
    ...
    if (query.level !== undefined && query.level !== '') {
      qb.andWhere('r.severity = :level', { level: Number(query.level) });
    }
    if (query.status !== undefined && query.status !== '') {
      qb.andWhere('r.status = :status', { status: Number(query.status) });
    }
    if (query.type) {
      qb.andWhere('r.alertType = :alertType', { alertType: Number(query.type) });
    }
    if (query.userId) {
  ...
  async getEventList(query: Record<string, any>): Promise<PaginatedResult<NftSecurityEvent>> {
    ...
    if (query.type) {
      qb.andWhere('e.eventType = :eventType', { eventType: Number(query.type) });
    }
    if (query.level !== undefined && query.level !== '') {
      qb.andWhere('e.eventLevel = :level', { level: Number(query.level) });
    }
