 ...
 *      APP_INTERCEPTOR  → LoggingInterceptor    全局请求日志拦截器（记录方法/路径/IP/耗时/状态码/Request ID）
...
@Module({
  imports: [
    ...
    // 公共模块
    // ------------------------------------------------------------
    HealthModule,         // 健康检查（/health, /health/live, /health/ready）
  ],
  providers: [
    ...
    // 全局请求日志拦截器（记录方法/路径/IP/耗时/状态码/Request ID，慢请求告警）
...
export class AppModule implements NestModule {
  configure(consumer: MiddlewareConsumer): void {
    // 注册 TraceId 中间件，为所有请求注入 traceId（AsyncLocalStorage）
